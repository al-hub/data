...
- 전략
```
  - 돌파(80~90년)는 돌파하면 우상향한다는 가정의 오롯한 매수기반 day-trading.
  - 장점: Basic통계검증, 일일현금화, 하락장매수방지   
  - 단점: 수수료세금(stock에안먹히는이유), 돌파후실패(즉,매도모델이없어돌파떨어짐대응x), 돌파시점delay시손해
  - (보완)K값:0.5 + 5/10평선, 평균, 일봉해석후대응(back테스트검증필-조합별정리), 누적된일봉대응(평균), 주간누적,월간누적,분기누적 (시스템상오류를 고려한 예외전략 반듯이필요)
  
  - (보완)비파: 쌀사없이도 동작되어야 한다.
  -             h1수수료와 세금이 기본이 되어야 한다.(안보이는 전기료도 있다 - -) 돌파의 힘은 가지고 있을 것이다 
  -             기본전략에서 min,max 실험치는 0.96 ~ 1.04 이다. 
  -             실전데이터에 1.00(0%이익)나는 경우 약 80% (지지는 않음)으로 타당성을 보이지만,
  -             +1%:6%, +2%:2%, +3%:0.5%, +4%:0.1% 로 현실적인 부분을 보여주고 있다.
  -             결국, 비파에서는 20%지는것을 감소하는것이 목적이 되는데,
  -             자칫 이익을 갉아 먹을 수 도 있다.
  -             +2%를 기준으로 이익을 버리면 0.6 ~ 12%까지 감소할수 있을것이다.(3~5%만 지는것을 감소시켜준다면 기존 20% -> 15~17% 막는다.)
  -             h2:2% 
  -             즉 1- (cur_prince / target) > h2 지점에서
  -             돌파력(QUEregressor)가 -기울기를 가지는 시점을 버리자.
